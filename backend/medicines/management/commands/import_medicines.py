...
class Command(BaseCommand):
    help = 'Импортирует лекарства в базу данных'

    def handle(self, *args, **options):
        self.stdout.write("Начинаем импорт лекарств...")
        created_count = 0
        updated_count = 0
        skipped_count = 0
        ...
        for data in medicines_data:
            ...
            try:
                ...
                if created:
                    created_count += 1
                    self.stdout.write(self.style.SUCCESS(f"✓ Создано: {medicine.name} ({article})"))
                else:
                    updated_count += 1
                    self.stdout.write(self.style.WARNING(f"◆ Обновлено: {medicine.name} ({article})"))
                    
            except Exception as e:
                skipped_count += 1
                self.stdout.write(self.style.ERROR(f"✗ Ошибка при импорте {data['name']}: {str(e)}"))
        ...
        self.stdout.write(self.style.ERROR(f"Ошибок: {skipped_count}"))
        self.stdout.write(f"Всего: {created_count + updated_count + skipped_count}")
